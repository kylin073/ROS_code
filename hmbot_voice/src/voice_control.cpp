...
#include "hmbot_voice/qtts.h"
#include "hmbot_voice/msp_cmn.h"
#include "hmbot_voice/msp_errors.h"
...
void control_turtle(float x_flag,float z_flag)
{
    int i =10;
	ros::Rate rate=(10.0);
	ros::NodeHandle n;
...
void voiceWordsCallback(const std_msgs::String::ConstPtr& msg )
{
    ...
    || dataString.find("向前") != std::string::npos)
    {
        ...
        char forwardString[100] = "向前移动小海龟";
        text = forwardString;
    ...
    {
        char turnleftString[100] = "向左移动小海龟";
        text = turnleftString;
        std::cout<<text<<std::endl;
		control_turtle(1,1);
         ...
         || dataString.find("向右") != std::string::npos)
    {
        char turnrightString[100] = "向右移动小海龟";
        text = turnrightString;
        std::cout<<text<<std::endl;
		control_turtle(1,-1);	
    }
    else
...
{
	int         ret                  = MSP_SUCCESS;
	const char* login_params         = "appid = 5d6394e2, work_dir = .";//登录参数,appid与msc库绑定,请勿随意改动
	/*
	* rdn:           合成音频数字发音方式
	* volume:        合成音频的音量
	* pitch:         合成音频的音调
