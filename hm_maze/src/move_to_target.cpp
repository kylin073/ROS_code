...
void poseCallback(const geometry_msgs::Pose::ConstPtr& msg)
{
    ...
    else if (status_flag == STATUS_CLOSE_TARGET)
    {
        ...
        vel_msg.linear.x = (120000 - msg->position.z)/120000 * 0.3;
        vel_msg.angular.z = (350 - msg->position.x)/350 * 0.3;

        vel_pub.publish(vel_msg);
        ROS_INFO("Publish velocity command [%0.2f m/s, %0.2f m/s]" , vel_msg.linear.x ,vel_msg.angular.z);
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber pose_sub = n.subscribe("/object_delect_pose", 10, poseCallback);
